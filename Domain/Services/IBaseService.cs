...
namespace Domain.Services
{
    public interface IBaseService
    {
        ...
        IQueryable<T> Query<T>(Expression<Func<T, bool>> predicate = null, bool readOnly = false) where T : class;
        T Insert<T>(T entity) where T : class;
        IEnumerable<T> InsertMany<T>(IEnumerable<T> entities) where T : class;
        T Update<T>(T entity) where T : class;
        IEnumerable<T> UpdateMany<T>(IEnumerable<T> entities) where T : class;
